 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class A24StudioCS_Sniffs_Commenting_FunctionCommentThrowTagSniff extends PHP_CodeSniffer_Standards_AbstractScopeSniff
{
    ...
    protected function processTokenWithinScope(PHP_CodeSniffer_File $phpcsFile, $stackPtr, $currScope)
    {
        ...
        if (empty($throws) === true) {
            ...
            if ($tokenCount !== $tagCount) {
                /*
                 * This has been added for exceptions that is set to a
                 * variable and thrown at a later stage.  Code sniffer
                 * can not check this kind of ecxeptions to be of the
                 * right type but can check that if a new exception is
                 * being thrown that it does have a @throws tag
                 */
                if ($tokenCount < $tagCount) {
                    foreach ($throwTokens as $throwToken) {
                        if (!in_array($throwToken, $throwTags)) {
                            $error = 'Expected %s to have a corresponding @throws tag';
                                $data  = array(
                                    $throwToken,
                                );
